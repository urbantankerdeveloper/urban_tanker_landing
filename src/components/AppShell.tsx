...
export function AppShell({ role, active, mobileNav, onRoleChange, onNavigate, onToggleMobileNav, onNotify, onSignOut, children }) {
  ...
  return <div className={`app role-${role}`}>
    ...
    <aside className={`sidebar ${mobileNav ? 'open' : ''}`}>
      <div className="sidebar-role"><span className="eyebrow">{content.operations.workspaceLabel}</span><select value={role} onChange={event => onRoleChange(event.target.value)}><option value="customer">{content.operations.customerView}</option><option value="vendor">{content.operations.vendorPortal}</option><option value="admin">{content.operations.adminCommandCentre}</option></select></div>
      <nav>{navItems.map(([id, label]) => <button className={active === id ? 'active' : ''} key={id} onClick={() => onNavigate(id)}><span className="nav-symbol" aria-hidden="true">{id === 'overview' ? '01' : id === 'book' ? '02' : id === 'orders' ? '03' : id === 'track' ? '04' : '05'}</span><span>{label}</span>{id === 'orders' && <b>4</b>}</button>)}</nav>
