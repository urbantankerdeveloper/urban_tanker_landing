...
export function CustomerHomeShell({ children, onNavigate, onSignOut }: CustomerHomeShellProps) {
  ...
  return <div className="customer-site">
    ...
    <header className="customer-header">
      ...
      <nav aria-label="Customer navigation"><button type="button" onClick={() => onNavigate('book')}>{content.customer.nav.offers}</button><button type="button" onClick={() => onNavigate('track')}>{content.customer.nav.track}</button><button type="button" onClick={() => onNavigate('support')}>{content.customer.nav.support}</button></nav>
    ...
    <footer className="customer-footer"><strong><span className="brand-mark"><Droplets size={14} /></span> {content.brand.name}</strong><p>{content.brand.tagline}</p><div><button type="button" onClick={() => onNavigate('track')}>{content.customer.nav.track}</button><button type="button" onClick={() => onNavigate('support')}>{content.customer.nav.support}</button></div></footer>
