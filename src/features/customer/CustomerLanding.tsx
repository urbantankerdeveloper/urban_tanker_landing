...
interface CustomerLandingProps { data: AppData; order?: Order; onNavigate: (workspace: Workspace) => void; onSelectService?: (service: BookingDraft['service']) => void; onBookAgain?: () => void; }

export function CustomerLanding({ data, order, onNavigate, onSelectService, onBookAgain }: CustomerLandingProps) {
  ...
  return <>
    ...
    <section className="customer-actions" aria-label="Book a service"><button type="button" onClick={() => onSelectService ? onSelectService('Water tanker') : onNavigate('book')}><span className="service-icon water"><Droplets size={22} /></span><span><b>{actions.water}</b><small>{actions.waterMeta}</small></span><ArrowRight size={18} /></button><button type="button" onClick={() => onSelectService ? onSelectService('Sewage pickup') : onNavigate('book')}><span className="service-icon sewage"><Activity size={22} /></span><span><b>{actions.sewage}</b><small>{actions.sewageMeta}</small></span><ArrowRight size={18} /></button>{order && <button className="repeat-action" type="button" onClick={onBookAgain}><span className="service-icon repeat"><RefreshCcw size={20} /></span><span><small>{actions.repeatMeta}</small><b>{actions.repeat} <em>₹{order.amount}</em></b></span><ArrowRight size={18} /></button>}</section>
    <section className="customer-booking-intro"><span className="eyebrow">{booking.eyebrow}</span><h2>{booking.title}</h2><p>{booking.description}</p><div className="booking-intro-actions"><Button variant="primary" onClick={() => onNavigate('offers')}>View Special Offers <ArrowRight size={16} /></Button><Button variant="quiet" onClick={() => onNavigate('book')}>{booking.cta} <ArrowRight size={16} /></Button></div></section>
