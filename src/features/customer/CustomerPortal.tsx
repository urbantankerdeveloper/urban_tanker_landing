...
import { loadCustomerOrders } from '../../shared/lib/cloudStore';
...
function BookingForm({ booking, set, onSubmit }: { booking: BookingDraft; set: <K extends keyof BookingDraft>(key: K, value: BookingDraft[K]) => void; onSubmit: () => void }) { const options = booking.service === 'Water tanker' ? ['Drinking / potable', 'Domestic', 'Borewell', 'Other'] : ['Septic tank cleaning', 'Sewage removal', 'Drain cleaning']; return <><PageHeader eyebrow="New booking" title="Plan your delivery in minutes." copy="Tell us what you need. We will surface the clearest available price before payment." /><div className="booking-layout"><form className="form-surface" onSubmit={event => { event.preventDefault(); onSubmit(); }}><div className="form-step"><span>01</span><div><h3>Choose your service</h3><p>We match capacity and vehicle type to your request.</p></div></div><div className="choice-grid two">{(['Water tanker', 'Sewage pickup'] as BookingDraft['service'][]).map(service => <button type="button" className={booking.service === service ? 'selected' : ''} onClick={() => set('service', service)} key={service}><Droplets size={22} /><b>{service}</b><small>{service === 'Water tanker' ? 'For homes, offices and sites' : 'Septic and wastewater removal'}</small></button>)}</div><div className="form-step"><span>02</span><div><h3>{booking.service === 'Water tanker' ? 'Water type' : 'Service requirement'}</h3><p>Help your driver arrive prepared.</p></div></div><div className="pill-row" role="group" aria-label="Service detail">{options.map(option => <button type="button" className={booking.waterType === option ? 'selected' : ''} onClick={() => set('waterType', option)} key={option}>{option}</button>)}</div><div className="form-step"><span>03</span><div><h3>Capacity and timing</h3><p>Select the right size for your location.</p></div></div><div className="capacity-row" role="group" aria-label="Tanker capacity">{Object.keys(prices).map(capacity => <button type="button" className={booking.capacity === capacity ? 'selected' : ''} onClick={() => set('capacity', capacity)} key={capacity}><b>{capacity.split(' ')[0]}</b><small>KL</small></button>)}</div><div className="field-row"><label><span>Delivery date</span><input value={booking.date} onChange={event => set('date', event.target.value)} /></label><label><span>Preferred slot</span><select value={booking.slot} onChange={event => set('slot', event.target.value)}><option>As soon as possible</option><option>Morning · 6 AM - 10 AM</option><option>Afternoon · 10 AM - 3 PM</option><option>Evening · 3 PM - 8 PM</option></select></label></div><div className="form-step"><span>04</span><div><h3>Where should we deliver?</h3><p>Give the driver enough detail for a smooth handover.</p></div></div><label htmlFor="delivery-address"><span>Delivery address</span><div className="input-icon"><MapPin size={16} /><input id="delivery-address" value={booking.address} onChange={event => set('address', event.target.value)} required /></div></label><div className="field-row"><label><span>Landmark <small>(optional)</small></span><input value={booking.landmark} onChange={event => set('landmark', event.target.value)} placeholder="Gate, metro or building" /></label><label><span>Special instructions <small>(optional)</small></span><input value={booking.notes} onChange={event => set('notes', event.target.value)} placeholder="Access details" /></label></div><Button type="submit" variant="primary full" icon={ArrowRight}>Continue to payment</Button></form><aside className="quote-card" aria-label="Booking estimate"><div className="quote-top"><span className="quote-icon"><IndianRupee size={19} /></span><div><span>Estimated price</span><strong>{money(prices[booking.capacity])}</strong></div></div><div className="quote-line"><span>Service</span><b>{booking.service}</b></div><div className="quote-line"><span>Capacity</span><b>{booking.capacity}</b></div><div className="quote-line"><span>Estimated arrival</span><b>35 - 50 min</b></div><div className="quote-assurance"><Check size={15} /> Final price shown before payment. No surprise charges.</div></aside></div></>; }
...
function OrdersView({ orders }: { orders: Order[] }) {
  ...
  return <><PageHeader eyebrow="Booking history" title="Your orders, together." copy="Active, upcoming, and completed water and wastewater services." action={<Button variant="quiet" icon={CalendarDays}>All time</Button>} /><div className="filter-tabs" role="tablist" aria-label="Order filters">{tabs.map(tab => <button className={filter === tab.id ? 'active' : ''} role="tab" aria-selected={filter === tab.id} onClick={() => { setFilter(tab.id); setPage(1); }} key={tab.id}>{tab.label} <b>{tab.count}</b></button>)}</div><div className="order-list">{visibleOrders.length ? pagedOrders.map(order => <article className="order-row" key={order.id}><div className="order-service-icon"><Droplets size={19} /></div><div className="order-main"><div><b>{order.service}</b><Status>{order.status}</Status></div><span>{order.id} · {order.capacity} · {order.address}</span><small>{order.created}</small></div><div className="order-amount"><strong>{money(order.amount)}</strong><span>{order.payment}</span></div><ArrowRight size={17} /></article>) : <div className="empty-state"><h3>No orders in this filter</h3><p>New bookings will appear here when they match this status.</p></div>}</div><Pagination page={page} pageSize={10} total={visibleOrders.length} onPageChange={setPage} /></>;
}
