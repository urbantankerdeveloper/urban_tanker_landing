...
export function VendorDriversWithProofView({ onNotify }: { onNotify: (message: string) => void }) {
  ...
  return <>
    ...
    <section className="stats-grid vendor-stats" aria-label="Driver activity summary">
      <StatCard icon={Users} label="Total drivers" value={String(drivers.length)} detail="Team roster" />
      <StatCard icon={Check} label="Active" value={String(activeDrivers)} detail="Ready for dispatch" tone="highlight" />
      <StatCard icon={ShieldCheck} label="Inactive" value={String(inactiveDrivers)} detail="Paused from assignment" />
    </section>
