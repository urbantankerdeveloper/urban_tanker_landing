import { Bell, CalendarCheck, ChevronDown, ClipboardList, Droplets, Gift, Headphones, LayoutDashboard, MapPin, Menu, PanelLeft, PanelLeftClose, Percent, Search, Settings2, Store, Truck, Users, WalletCards, Wrench, type LucideIcon } from 'lucide-react';
...
export function AppShell({ role, active, orderCount = 0, mobileNav, onNavigate, onToggleMobileNav, onNotify, onSignOut, children }) {
  ...
  const vendorWorkforceFeaturesEnabled = import.meta.env.VITE_ENABLE_VENDOR_WORKFORCE_FEATURES === 'true';
  const dispatchBoardEnabled = import.meta.env.VITE_ENABLE_DISPATCH_BOARD === 'true';
  ...
  const navItems = [["overview", content.operations.overview, LayoutDashboard], ...(role === 'customer' ? [["book", content.operations.bookTanker, Truck]] : []), ["orders", content.operations.orders, ClipboardList], ["track", content.operations.liveTracking, MapPin], ...(role === 'customer' ? [["subscriptions", 'Subscriptions', CalendarCheck], ["invoices", 'Invoices & payments', WalletCards]] : []), ...(role === 'vendor' ? [["fleet", 'Fleet', Truck], ["drivers", 'Drivers', Users], ["maintenance", 'Maintenance', Wrench], ...(vendorWorkforceFeaturesEnabled ? [["attendance", 'Attendance', CalendarCheck], ["payouts", 'Payouts', WalletCards]] : [])] : []), ...(role === 'admin' ? [...(dispatchBoardEnabled ? [["dispatch", 'Dispatch board', ClipboardList]] : []), ["customers", 'Customers', Users], ["vendors", 'Vendors', Store], ["fleet", 'Fleet', Truck], ["drivers", 'Drivers', Users], ["coupons", 'Coupons', Percent], ["offers-management", 'Offers', Gift]] : []), ["support", content.operations.helpSupport, Headphones]] as Array<[Workspace, string, LucideIcon]>;
